...
async def add_review_to_db(
    session: AsyncSession, new_review: ReviewCreateSchema
) -> ReviewModel:
    db_new_review = ReviewModel(
        text=new_review.text,
        # Задача является CPU-Bound и блокирует асинхронное выполнения
        # Так как размер отзыва ограничен (5000 символов) в данном случает
        # блокировка будет не большой (её можно не учитывать)
        # При увеличении количества символов необходимо рассмотреть другой
        # способ определения sentiment
...
async def get_reviews_from_db_by_sentiment(
    session: AsyncSession, sentiment: str
